...
class PredjspiderSpider(scrapy.Spider):
    name = 'predjspider8'
    allowed_domains = ['www.vfast.org/journals/index.php/VTSE/article/view/']
    ASIN = list(range(20,552))
    ASIN = [str(i) for i in ASIN]

    def start_requests(self):
        for i in self.ASIN:
            req = Request('http://www.vfast.org/journals/index.php/VTSE/article/view/'+i,callback = self.parse_article)
            # Wait for 5 seconds
          #  time.sleep(.5)
            yield req
