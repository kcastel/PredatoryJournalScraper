...
class PredjspiderSpider(scrapy.Spider):
    name = 'predjspider10'
    allowed_domains = ['www.vfast.org/journals/index.php/VTM/article/view/']
    ASIN = list(range(22,418))
    ASIN = [str(i) for i in ASIN]

    def start_requests(self):
        for i in self.ASIN:
            req = Request('http://www.vfast.org/journals/index.php/VTM/article/view/'+i,callback = self.parse_article)
            # Wait for 5 seconds
          #  time.sleep(.5)
            yield req
