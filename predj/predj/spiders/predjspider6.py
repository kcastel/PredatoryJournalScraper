...
class PredjspiderSpider(scrapy.Spider):
    name = 'predjspider6'
    allowed_domains = ['www.theinternationaljournal.org/ojs/index.php?journal=rjitsm&page=article&op=view&path%5B%5D=']
    ASIN = list(range(346,7048))
    ASIN = [str(i) for i in ASIN]

    def start_requests(self):
        for i in self.ASIN:
            req = Request('http://www.theinternationaljournal.org/ojs/index.php?journal=rjitsm&page=article&op=view&path%5B%5D='+i,callback = self.parse_article)
            # Wait for 5 seconds
          #  time.sleep(.5)
            yield req
